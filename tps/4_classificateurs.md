...
Elle renvoie un DataFrame contenant tous les résultats/performances des tests sur les classificateurs.
...
plt.xlabel(sklearn_metric.__name__)
...
Les classificateurs qui ont sous appris sont: Nearest neighbors, Parzen Windows, Linear SVM, Naive Bayes et QDA, il n'y en a aucun qui a surappris.
...
tags: []
---
#performances = commitee.score(compar_results.perf_te, compar_results.perf_tr)

"""
X = df_features.iloc[:, :-1].to_numpy()
Y = df_features.iloc[:, -1].to_numpy()
performances = commitee.score(X, Y)
"""
```
...
image_grid(certain_aleatoric_images[48:58])
```
...
Ces résultats semblent cohérents avec notre base de données.
...
nbgrader:
  ...
  task: false
---
certain_epistemic_images = df_uncertainty['images'].tolist()
image_grid(certain_epistemic_images[48:58])
```
...
Ces résultats semblent s'opposer à ceux trouver avec l'utilisation de 'aleatoric'.
:::
...
Ce réseau a 32228864 paramètres, parmi lesquels 3206976 sont entraînables.
...
Ce réseau a à présent 3598754 paramètres, parmi lesquels 69890 sont entraînables.
...
On a appris à utiliser différents classificateurs selon leurs performances, et comment savoir quels classificateurs étaient plus adéquats dans certaines situations. On a aussi très rapidement observé comment fonctionnait un réseau de neurones.
