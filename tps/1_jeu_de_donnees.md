...
+++ {"deletable": false, "editable": false, "nbgrader": {"cell_type": "markdown", "checksum": "f96bf6d6bcbafe1b814d4970f60e6f62", "grade": false, "grade_id": "cell-4911a792a82448d7", "locked": true, "schema_version": 3, "solution": false}}

# Création de votre jeu de données pour le projet 2

+++ {"deletable": false, "editable": false, "nbgrader": {"cell_type": "markdown", "checksum": "3759f908ace83d4d5fcff2454b0c696e", "grade": false, "grade_id": "cell-4911a792a82448d8", "locked": true, "schema_version": 3, "solution": false}}
...
+++ {"deletable": false, "editable": false, "nbgrader": {"cell_type": "markdown", "checksum": "db4629f4c1e1099dd2b7feea562ffe83", "grade": false, "grade_id": "cell-4911a792a82448d9", "locked": true, "schema_version": 3, "solution": false}}
...
- [ ] Vérifiez la taille individuelle des images en (1) ouvrant un terminal et (2) en tapant:
    ...
    **Si elle dépasse 50ko par image**, réduisez la résolution pour
    ...
    Vous pourrez ensuite les redimensionner et les recadrer en Python comme ce qu'on a vu en TP6.
...
## Ajout des images au dépot git

Vous allez ajouter vos images au dépôt `git` dans le dossier `data/`:

```{code-cell} ipython3
! git pull #je vérifie que mon dossier est à jour
! git add data/* #j'ajoute à la liste de mes fichiers git les images
! git commit -m "ajout des images" #j'ecris un commentaire pour me souvenir ce que je viens de faire dans l'historique
! git config --global user.name "jeremy.rakotonirina" user.email="jeremy.rakotonirina@universite-paris-saclay.fr"; git push

#XXTODO le git config ne fonctionne pas, il ne se passe rien et je n'ai pas d'username Username for 'https://gitlab.dsi.universite-paris-saclay.fr': 
```
...
Après avoir déposé vos images, et éventuellement ajusté leur extension
à la ligne suivante, lancez les tests qui suivent pour quelques
vérifications.
...
dataset_dir = os.path.join('..','data2')
